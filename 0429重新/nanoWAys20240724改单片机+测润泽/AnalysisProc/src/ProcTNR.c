...
//	AGENT_CODE agentCode = AGENT_SAMPLE;
	...
	if(pumpTime != 0)
	{
		OpenFunValve(dev,F_VALVE_WATER_PUMP);//关水泵
...
//		return false;	
	if(false == ProcAddAgentToPool(dev,AGENT_DISTILLED_WATER,12,true)) return false; //加蒸馏水	3mL 20190519
	...
	switch(range)
	{
		case 0:  //TNR 0-5 20190519
			if(false == ProcAddAgentToPool(dev,agentCode,3,true)) //加水样  4mL -20190519     
				return false;
			else
		...
		//	ProcWashTubeWithAgent(dev,AGENT_DISTILLED_WATER,3,false);				//洗定量管
		//	if(false == ProcAddAgentToPool(dev,AGENT_DISTILLED_WATER,9,true))return false; //加蒸馏水	
		//  ProcDrawPool(dev,CLEAN_WASTE_VALVE,8);
	...
	ProcDelay(1);
	ProcPreagentPreWashTube(dev,AGENT_OXIDANT,1,false);								//190429 增加
...
//	OpenFunValve(dev,F_VALVE_FAN);    //开风扇  20210505
	ProcUpdateStepStatus((STEP_NAME_CODE)F_VALVE_SULPHURIC_ACID);	//加硫酸20190519
	ProcWashTubeWithAgent(dev,AGENT_DISTILLED_WATER,1,false);				//纯水洗定量管 20190519
	//ProcEmptyTube(dev,CLEAN_WASTE_VALVE);  //清空定量管
	//ProcEmptyTube(dev,F_VALVE_CLEAN_WASTE);  //清空定量管
	if(false == ProcAddAgentToPool(dev,(AGENT_CODE)F_VALVE_SULPHURIC_ACID,6,true))return false; //加显色剂-对应加硫酸 7mL 20190519
	ProcAirPool(dev);																									  //向反应池吹气
		
	ProcAirPool(dev); //向池子吹气
	//ProcEmptyTube(dev,F_VALVE_CLEAN_WASTE); //清定量管
//	CloseFunValve(dev,F_VALVE_FAN);      //关风扇 20210505	
ProcUpdateStepStatus(STEP_HOTTING);
  ProcTempCtrl(true,TEMP_CTRL_CH_POOL,70,5);//开启加热控制
	
	ProcUpdateStepStatus(STEP_READ_REF);																							//更新进度条名称
	ProcDelay(30);
	ProcReadBlankADs(10);

	ProcUpdateStepStatus((STEP_NAME_CODE)F_VALVE_CHROMOGENIC); //加间苯二酚
	ProcDelay(1);
	ProcPreagentPreWashTube(dev,AGENT_CHROMOGENIC,1,false);        //190429 增加

	if(false == ProcAddAgentToPool(dev,(AGENT_CODE)F_VALVE_CHROMOGENIC,2,true))//显色剂 对应间苯二酚显色剂	20190519
	  return false;
	
	ProcUpdateStepStatus(STEP_HOTTING);
  ProcTempCtrl(true,TEMP_CTRL_CH_POOL,70,3);//开启加热控制
