...
//		return false ;
	...
	switch(range)
	{
		...
		case 1:
			...
			ProcDrawPool(dev,CLEAN_WASTE_VALVE,6);
			ProcAirPool(dev);
			ProcWashTubeWithAgent(dev,AGENT_DISTILLED_WATER,3,false);				//洗定量管
			ProcWashTubeWithAgent(dev,AGENT_DISTILLED_WATER,3,false);				//洗定量管
	...
	ProcDelay(1);	
	if(false == ProcAddAgentToPool(dev,AGENT_NEUTRALIZER,3,true))//加中和剂
		return false;
	ProcAirPool(dev); //向池子吹气
	ProcUpdateStepStatus(STEP_COOLING);
	ProcDelay(1);
...
//	ProcReadReactOOpUVData();
	...
	ProcDelay(1);
	ProcWashTubeWithAgent(dev,AGENT_DISTILLED_WATER,3,false);    //洗定量管
	if(false == ProcAddAgentToPool(dev,AGENT_NEUTRALIZER,3,false))return false;//加中和剂
