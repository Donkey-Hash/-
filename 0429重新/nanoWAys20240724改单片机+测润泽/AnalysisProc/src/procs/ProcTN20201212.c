...
//	OOpInit(5,220,1);
	...
	if(pumpTime != 0)
	{
		...
		OpenFunValve(dev,WATER_PUMP_VALVE);				//开水泵
...
//		return false ;
	...
	switch(range)
	{
		case 0:
			...
			ProcWashTubeWithAgent(dev,agentCode,3,false);				
			if(false == ProcAddAgentToPool(dev,agentCode,4,true)) //加水样            3mL改为4mL-志强090427
				return false; 
		...
		case 2:
			...
			ProcDrawPool(dev,CLEAN_WASTE_VALVE,9);                //6改成9 -志强090427
			ProcAirPool(dev);
			ProcWashTubeWithAgent(dev,AGENT_DISTILLED_WATER,1,false);				//洗定量管  3mL改为1mL-志强090427
			ProcWashTubeWithAgent(dev,AGENT_DISTILLED_WATER,3,false);				//洗定量管
			if(false == ProcAddAgentToPool(dev,AGENT_DISTILLED_WATER,9,true))return false; //加蒸馏水   //6改成9 -志强090427
			break;
