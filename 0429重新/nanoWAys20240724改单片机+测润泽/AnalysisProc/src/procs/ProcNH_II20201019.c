...
//			ProcWashTubeWithAgent(dev,agentCode,3,false);													//洗定量管
//			ProcWashTubeWithAgent(dev,agentCode,3,false);													//洗定量管
			if(false == ProcAddAgentToPool(dev,agentCode,9,true))return false; 		//加水样	增加3毫升水样2020529
			ProcFillLoop(dev);    																								//定量环定量。
			ProcWashTubeWithAgent(dev,AGENT_DISTILLED_WATER,3,false);							//洗定量管
			ProcWashPoolWithAgent(dev,AGENT_DISTILLED_WATER,9); 									//清洗比色池
			ProcAirLoopPool(dev);    																							//通过定量环向比色池吹气
			if(false == ProcAddAgentLoopPool(dev,AGENT_DISTILLED_WATER,12,true))return false; //通过定量环向反应池加零点液
			ProcAirLoopPool(dev);    																							//通过定量环向比色池吹气
			ProcAirLoopPool(dev);    																							//通过定量环向比色池吹气
			ProcFillLoop(dev);																										//定量环定量
			ProcWashTubeWithAgent(dev,AGENT_DISTILLED_WATER,3,false);							//洗定量管
			ProcWashPoolWithAgent(dev,AGENT_DISTILLED_WATER,9);										//清洗比色池
			ProcAirLoopPool(dev);    																							//通过定量环向比色池吹气
			if(false == ProcAddAgentLoopPool(dev,AGENT_DISTILLED_WATER,10,true))return false; //通过定量环向反应池加零点液
		break;
		default:break;
		}
		if(pumpTime != 0)
