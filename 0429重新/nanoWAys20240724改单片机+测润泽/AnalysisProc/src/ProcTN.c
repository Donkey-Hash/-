...
////		return false ;
	...
	switch(range)
	{
		...
		case 2:
			...
			ProcWashTubeWithAgent(dev,AGENT_DISTILLED_WATER,1,false);				//洗定量管  3mL改为1mL-志强190427
			ProcWashTubeWithAgent(dev,AGENT_DISTILLED_WATER,3,false);				//洗定量管
			if(false == ProcAddAgentToPool(dev,AGENT_DISTILLED_WATER,12,true))return false; //加蒸馏水 9改成12mL20201219
			ProcDrawPool(dev,F_VALVE_CLEAN_WASTE,10);                //
			ProcWashTubeWithAgent(dev,AGENT_DISTILLED_WATER,1,false);				//洗定量管  3mL改为1mL-志强190427
			ProcWashTubeWithAgent(dev,AGENT_DISTILLED_WATER,3,false);				//洗定量管
	...
	ProcEmptyTube(dev,F_VALVE_CLEAN_WASTE);				//排空定量管//190417 增加
//	ProcPushBackAgent(dev,agentCode);						//回推水样管//190417 增加
