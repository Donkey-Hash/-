...
#define COEF_OVERFLOW   0.75    //过冲系数 单位：圈
...
{
	uint32_t timeout = (maxcircle * 600 * 1000)/speed;      //speed:0.1rpm,timeout: ms
	if(dir == CW)
		{PUMP_DIE_CW;}
	else if(dir == CCW)
		{PUMP_DIE_CCW;}
...
* Return         : 对应通道AD值
*******************************************************************************/
void ChangePumpSpeed(uint16_t speed)
{
	StartStepMotorByFrequency(speed);		
}
/*******************************************************************************
...
//	TotalRefTHR = (EmptyLLAD/2);											//设置全反射点AD阈值
	do{
		...
		PracticalLLAD = *(&(pMbHoldData->nLL1AD) + ch);							//读取当前值
		SampleCount++;
		switch(LLCheckState)
		{
			case 0:
...
//	uint16_t speed = PumpAgentSpeed[agent];
	uint16_t speed = GetSpeed(dev,(FUN_VALVE_CODE)agent);
	ret = DrawLiquid(dev,(FUN_VALVE_CODE)agent,speed,quanty,fine);
	if(DRAW_OK != ret)
	{
		SendAir(dev,F_VALVE_CLEAN_WASTE,MAX_PULL_LIQUID_CIRCLE/2);  //清空定量管
		ret = DrawLiquid(dev,(FUN_VALVE_CODE)agent,0,quanty,fine);
...
DrawLiquidERROR TrsLiquidByTube( DEVICE_CODE dev,FUN_VALVE_CODE src,FUN_VALVE_CODE des,
																uint16_t quanty,bool fine)
{
	DrawLiquidERROR ret = DRAW_OK;
//	quanty /= 1000;
	uint8_t lq = quanty / 3;						//高液位需要定量的次数
	uint8_t sq = quanty % 3;						//低液位需要定量的次数
	uint16_t speed = GetSpeed(dev,src);
	while(sq--)   											//先抽取小液位定量
	{
		if(false == WashPipe(dev,SPEED_REACTION_POOL_SOLUTION,1))
		{
//			ReplyFaultInProcess(LL1_LIGHT_FAULT);
			return DETECT_LEVEL1_FALSE;
		}		
	...
	while(lq--)
	{
		...
		{
//			ReplyFaultInProcess(LL2_LIGHT_FAULT);
			return DETECT_LEVEL2_FALSE;
		}
...
* Return         	:
*******************************************************************************/
void SendAir(DEVICE_CODE dev,FUN_VALVE_CODE valve,uint8_t maxcircle)
{
	uint16_t speed = GetSpeed(dev,valve);
...
DrawLiquidERROR PoolEmptying(DEVICE_CODE dev,FUN_VALVE_CODE des)
{
	...
	while(ret == DRAW_OK)
	{
		ret = TrsLiquidByTube(dev,F_VALVE_POOL_ONLY,des,3,false);	
		if(ret != DRAW_OK)
			break;
		times++;
		if( times % 5 == 0)
		{
			if(false == WashPipe(dev,SPEED_REACTION_POOL_SOLUTION/2,3))
			{
				return DETECT_LEVEL1_FALSE;
			}
		}
	...
	SendAir(dev,des,MAX_PUSH_LIQUID_CIRCLE);
	return ret;
}
/*******************************************************************************
...
FAULT_CODE FillLoop(DEVICE_CODE dev)
{
	DrawLiquidERROR ret = DRAW_OK;
	FAULT_CODE fault;
	ret = TrsLiquidByTube(dev,F_VALVE_LOOP_ONLINE_POOL,F_VALVE_CLEAN_WASTE,1,false);
	ret = TrsLiquidByTube(dev,F_VALVE_LOOP_ONLINE_POOL,F_VALVE_CLEAN_WASTE,1,false);
	if(ret == DRAW_OK)
	{
...
//	uint16_t speed = PumpAgentSpeed[agent];
	...
	if(fun != NULL)
	ret = fun(dev,(FUN_VALVE_CODE)agent,throughvalve,quanty,fine);
	else
		ret = NO_DRIV_FUN;
...
//	uint16_t speed = PumpAgentSpeed[agent];
	...
	fun = GetLiquidFun(dev,(FUN_VALVE_CODE)agent);
	if(fun != NULL)
	ret = fun(dev,(FUN_VALVE_CODE)agent,F_VALVE_POOL_ONLY,quanty,fine);
	else
		ret = NO_DRIV_FUN;
	fault = ErrorCodeConvert(ret,agent);
	return fault;
...
//	uint16_t speed = PumpAgentSpeed[agent];
	...
	ret = TrsLiquidByTube(dev,(FUN_VALVE_CODE)agent,F_VALVE_LOOP_ONLINE_POOL,quanty,fine);
	fault = ErrorCodeConvert(ret,agent);
	return fault;
}
...
FAULT_CODE WashPoolWithAgent(DEVICE_CODE dev,AGENT_CODE agent,uint16_t quanty)
{
	...
	{
		SendAir(dev,F_VALVE_POOL_ONLY,6);
		PoolEmptying(dev,F_VALVE_CLEAN_WASTE);
	}
	else
	{	
//		ReplyFaultInProcess(faultcode);
//		SendAir(dev,F_VALVE_CLEAN_WASTE,SPEED_DEFAULT,3);
//		PoolEmptying(dev,F_VALVE_CLEAN_WASTE);
	}
...
* Return         : 完成状态
*******************************************************************************/
FAULT_CODE WashPool(DEVICE_CODE dev,AGENT_CODE agent,uint16_t quanty,FUN_VALVE_CODE des)
{
	FAULT_CODE faultcode = NO_FAULT;
	...
	{
		SendAir(dev,F_VALVE_POOL_ONLY,6);
		PoolEmptying(dev,des);
	}
	else
	{	
//		ReplyFaultInProcess(faultcode);
//		SendAir(dev,F_VALVE_CLEAN_WASTE,3);
//		PoolEmptying(dev,des);
	}
...
/*******************************************************************************
* Function Name  : WashPoolWithAgentByPump
* Description    : 清洗反应池，排液泵排液
* Input          : 	valve:试剂对应的电磁阀; speed:加试剂速度；
										maxcircle：吹气圈数（蠕动泵）
* Output         : qdone:已经加液量
* Return         : 完成状态
*******************************************************************************/
FAULT_CODE WashPoolWithAgentByPump(DEVICE_CODE dev,AGENT_CODE agent,uint16_t quanty,FUN_VALVE_CODE des,uint16_t timeSec)
{
	FAULT_CODE faultcode = NO_FAULT;
...
//	times = quanty / 2.5;
	...
	return DRAW_OK;
//	ProcTitraPump(dev,F_VALVE_TITRATION_PUMP,1000);      //滴定泵加氧化剂
}
/*******************************************************************************
* Function Name  : 溢流进样
* Description    : 抽取液体
