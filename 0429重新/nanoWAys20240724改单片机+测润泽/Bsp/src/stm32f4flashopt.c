...
//addr:flash地址
//返回值:0~11,即addr所在的扇区
static uint8_t Stm32F4GetFlashSector(uint32_t addr)
{
...
//获取某个地址所在的flash扇区内存大小
//addr:flash地址
//返回值:0~11,即addr所在的扇区
uint32_t Stm32F4GetSector(uint8_t sector)
{
	switch(sector)
	{
		case FLASH_SECTOR_0:
...
{ 
	...
	uint32_t addrx=0;
	uint32_t endaddr=0;	
	if(WriteAddr<STM32_FLASH_BASE||WriteAddr%4)return HAL_BUSY;	//非法地址
	...
	if(addrx<0X1FFF0000)
	{
		...
		{
			if(Stm32ReadWord(addrx)!=0XFFFFFFFF)//有非0XFFFFFFFF的地方,要擦除这个扇区
				{   
					FlashEraseInit.TypeErase=FLASH_TYPEERASE_SECTORS;       //擦除类型，扇区擦除 
					FlashEraseInit.Sector=Stm32F4GetFlashSector(addrx);   //要擦除的扇区
...
void Stm32f4ReadWords(uint32_t ReadAddr,uint32_t *pBuffer,uint32_t NumToRead)   	
{
	...
	{
		pBuffer[i]=Stm32ReadWord(ReadAddr);//读取4个字节.
		ReadAddr+=4;//偏移4个字节.	
	}
}
...
{
	Stm32f4WriteWords(WriteAddr,&WriteData,1);//写入一个字 
}
